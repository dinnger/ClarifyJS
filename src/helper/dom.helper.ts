...
export class DOMHelper {
  ...
  static showFieldError(
    ...
    const input = container.querySelector(`[name="${fieldPath}"]`) as 
  ...
  static applyErrorStyles(
    ...
    errors.forEach((err) => {
      err.path.forEach((key: string) => {
        ...
        const input = container.querySelector(`[name="${key}"]`) as 
          HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
  ...
  static updateFieldProperty(
    ...
    switch (property) {
      ...
      case "disabled":
        const input = fieldElement.querySelector("input, textarea, select") as 
          HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
        if (input) {
          if (value) {
